...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    for i in tqdm(range(n_batches)):
        ...
        if is_numpy:
            ...
            images = [np.array(Image.open(str(f)).convert('RGB').resize((256, 256))) for f in files[start:end]]
...
def _compute_activations(path, model, batch_size, dims, cuda, model_type, subset=None):
    ...
    if subset is not None:
        ...
        subset_indices = np.random.choice(len(path), subset, replace=False)
        path = [path[i] for i in subset_indices]
        assert len(path) == subset
...
def calculate_fid_given_paths(paths, batch_size, cuda, dims, bootstrap=True, n_bootstraps=10, model_type='inception', use_rad_imagenet_features=False, subset=None):
    ...
    act_true = _compute_activations(pths[0], model, batch_size, dims, cuda, model_type, subset=subset)
    act_fake = []
    act_true_pth = os.path.join(pths[0], '{}_acts.npy'.format('FID' if not use_rad_imagenet_features else 'RadFID'))
    act_fake_pth = os.path.join(pths[1], '{}_acts.npy'.format('FID' if not use_rad_imagenet_features else 'RadFID'))
    n_bootstraps = n_bootstraps if bootstrap else 1
    pths = pths[1:]
    results = []
    for j, pth in enumerate(pths):
        print(paths[j+1])
        actj = _compute_activations(pth, model, batch_size, dims, cuda, model_type, subset=subset)
        act_fake.append(actj)
    ...
    if save_activations:
        ...
        np.save(act_true_pth, act_true)
        np.save(act_fake_pth, act_fake)
